...
export function OrderTaxLink({orderId}:{orderId?:number}){
 ...
 return <div className="no-print my-3 text-sm"><Link className="border rounded px-3 py-2 inline-block" href={'/admin/tax-invoices?environment=production'+(orderId?'&order='+orderId:'')}>{orderId?'이 주문 세금계산서':'미발행 주문 · 일괄 발행'}</Link>{orderId&&<span className="ml-3">{q.isLoading?'상태 확인 중…':q.isError?'상태 조회 실패':row?labels[row.state]||row.state:'처리 완료 후 발행 가능'}</span>}</div>;
}
