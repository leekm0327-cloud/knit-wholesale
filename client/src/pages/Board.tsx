...
export default function Board() {
  ...
  // 공지 탭은 관리자만 작성 가능
  const canWrite = tab === "notice" ? isAdmin : !!user;
        ...
        <div className="mb-5 flex items-center justify-between border-b border-border">
          ...
          {canWrite ? (
            ...
            <div className="py-3 text-[11px] text-muted-foreground">
              {tab === "notice" ? "공지는 관리자만 등록할 수 있습니다." : ""}
            </div>
          )}
        </div>
...
function PostDetailDialog({
  ...
  const canDeletePost = data && (isAdmin || data.authorId === currentUserId);
