...
import { supplyLabels, type SupplyState, type SupplyRecord, type SupplyTemplate, type SupplyEvent } from '@shared/supply-workflow';
...
function Record({ r, me, onEdit, onConvert, onCopy, onTemplate, invalidate }: {
 ...
 <div className="flex flex-wrap gap-2 mt-3">{r.status === 'needed' && <button className="s-pill" onClick={onConvert}>발주했어요</button>}{(actions[r.status] || []).map(s => <button key={s} className="s-pill line" disabled={busy} onClick={() => { setTarget(s); setNote(''); }}>{supplyLabels[s]}</button>)}</div>
