...
export function TestLedgerSummary(){
 const q=useQuery<{expenses:{category:string;sector:string;amount:number;count:number}[];payments:{customerId:number;name:string;amount:number;count:number}[]}>({queryKey:['/api/admin/bank-test-ledger'],staleTime:0});
 if(q.isError)return <p role="alert">테스트 장부 요약을 불러오지 못했습니다.</p>;
 return <section className="border rounded p-4 my-4"><h2 className="font-semibold">테스트 장부 반영 현황</h2><p className="text-sm text-gray-600">운영 재무제표·거래처 잔액에는 포함되지 않습니다.</p>{q.isLoading?<p>불러오는 중…</p>:<div className="grid md:grid-cols-2 gap-4 mt-3"><div><h3>추가 비용</h3>{q.data?.expenses.map((x,i)=><p key={i}>{x.category} · {x.sector} · {currency(x.amount)} ({x.count}건)</p>)}{!q.data?.expenses.length&&<p>없음</p>}</div><div><h3>거래처별 미수금 차감 예정액</h3>{q.data?.payments.map(x=><p key={x.customerId}>{x.name} · −{currency(x.amount)} ({x.count}건)</p>)}{!q.data?.payments.length&&<p>없음</p>}</div></div>}</section>;
}
export default function BankPosting({row,customers,onChange}:{row:{id:number;deposit:number;withdraw:number;state:string;targetId:number|null};customers:{id:number;name:string}[];onChange:()=>Promise<unknown>}){
 ...
 const q=useQuery<{history:Posting[];categories:{name:string;sector:string}[]}>({queryKey:[`/api/admin/bank-review/${row.id}/posting`],staleTime:0});
 const active=q.data?.history.find(x=>!x.cancelled_at),expense=row.withdraw>0;
 const allowed=!['expense','payment','transfer','card','loan'].includes(row.state)&&!(row.deposit>0&&row.withdraw>0);
 ...
 return <section className="border-t mt-5 pt-4"><h3 className="font-semibold">장부 반영 테스트</h3>{q.isLoading?<p>반영 기록 확인 중…</p>:q.isError?<p role="alert">반영 기록을 불러오지 못했습니다. <button onClick={()=>q.refetch()}>다시 시도</button></p>:<>
 {active?<div><p className="my-3">테스트 반영 완료 · {currency(active.amount)} · {active.kind==='expense'?active.category:customers.find(c=>c.id===active.customer_id)?.name||'거래처 수금'}</p><input className="border p-2" aria-label="반영 취소 사유" placeholder="취소 사유" maxLength={500} value={reason} onChange={e=>setReason(e.target.value)}/><button className="border p-2 ml-2" disabled={busy||!reason.trim()} onClick={()=>run(()=>apiRequest('POST',`/api/admin/bank-review/${row.id}/posting/${active.id}/cancel`,{reason}))}>테스트 반영 취소</button></div>:!allowed?<p className="my-3">현재 분류는 비용·수금을 새로 생성하지 않습니다.</p>:<>
 <p className="my-3">{expense?'비용 추가':'미수금 차감 예정'} <strong>{currency(expense?row.withdraw:row.deposit)}</strong></p>
 ...
 <button className="border rounded px-4 py-2" disabled={busy||!checked||(expense?!(category&&sector):!customer)} onClick={()=>run(()=>apiRequest('POST',`/api/admin/bank-review/${row.id}/posting`,{kind:expense?'expense':'payment',...(expense?{category,sector}:{customerId:Number(customer)}),memo,duplicateChecked:true}))}>{busy?'처리 중…':'테스트 장부에 반영'}</button>
 </>}
