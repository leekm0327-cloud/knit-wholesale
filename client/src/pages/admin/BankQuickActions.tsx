...
export default function BankQuickActions({row,live,customers,categories,onChange,onDetails}:{row:Row;live:boolean;customers:{id:number;name:string}[];categories:{name:string;sector:string}[];onChange:()=>Promise<unknown>;onDetails:()=>void}){
 ...
 {!expense&&!row.posted&&<select aria-label="정산 입금 분류" disabled={busy} value={['settlement','online','delivery'].includes(row.state)?row.state:''} onChange={e=>{const state=e.target.value;if(state)run(()=>apiRequest('PATCH',`${base}/${row.id}`,{state,targetId:null,memo:row.memo}));}}><option value="">정산 입금 분류</option><option value="settlement">매장 정산</option><option value="online">온라인 정산</option><option value="delivery">배달 정산</option></select>}
 <button className="bank-quick-detail" disabled={busy} onClick={onDetails}>{row.posted?'반영 내역 · 취소':row.state==='online'?'매출 연결':'상세 처리'}</button>{error&&<p role="alert">{error}</p>}
 </div>;
}
