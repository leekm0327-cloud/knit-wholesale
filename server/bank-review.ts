...
async function soap(method:string,fields:Record<string,string|number>){
 ...
 try{response=await fetch(endpoint,{method:'POST',headers:{'Content-Type':'text/xml; charset=utf-8',SOAPAction:`"${namespace}${method}"`},body,signal:AbortSignal.timeout(25000),redirect:'error'});}catch{throw new Error('바로빌 응답을 받지 못했습니다. 잠시 후 다시 시도해 주세요.');}
...
export async function fetchBankRows(from:string,to:string){
 range.parse({from,to});const c=bankConfig();const fields={CERTKEY:c.key,CorpNum:c.corp};
 const accounts=list((await soap('GetBankAccount',fields)).BankAccount);
 ...
 for(const account of accounts){
  ...
  for(let page=1;page<=200;page++){
   const result=await soap('GetPeriodBankAccountTransLog',{...fields,ID:c.id,BankAccountNum:account.BankAccountNum,StartDate:from.replaceAll('-',''),EndDate:to.replaceAll('-',''),TransDirection:1,CountPerPage:100,CurrentPage:page,OrderDirection:1});
...
export function registerBankReview(app:Express,db:Database.Database,owner:RequestHandler){
 ...
 registerBankPosting(app,db,owner);
 ...
 app.get('/api/admin/bank-review',owner,safe((_req,res)=>{
  res.json({environment:'test',configured:!!(process.env.BAROBILL_TEST_KEY&&process.env.BAROBILL_CORP_NUM&&process.env.BAROBILL_USER_ID),rows:db.prepare("SELECT id,at,deposit,withdraw,remark,state,target_id AS targetId,memo,EXISTS(SELECT 1 FROM bank_test_postings p WHERE p.bank_id=bank_review.id AND p.cancelled_at IS NULL) AS posted,'****'||substr(account,-4) AS account FROM bank_review WHERE environment='test' ORDER BY at DESC,id DESC").all()});
 }));
 app.post('/api/admin/bank-review/import',owner,safe((req,res)=>{
 ...
 let syncing=false;
 app.post('/api/admin/bank-review/sync',owner,safe(async(req,res)=>{
  const {from,to}=range.parse(req.body);if(syncing)return res.status(409).json({message:'이미 내역을 가져오는 중입니다.'});syncing=true;
  try{const rows=await fetchBankRows(from,to);let added=0;
   db.transaction(()=>{const insert=db.prepare('INSERT OR IGNORE INTO bank_review(account,ref,at,deposit,withdraw,remark,currency) VALUES(?,?,?,?,?,?,?)');for(const r of rows)added+=insert.run(r.account,r.ref,r.at,r.deposit,r.withdraw,r.remark,r.currency).changes;})();
   res.json({received:rows.length,added});
  }finally{syncing=false;}
 }));
 app.get('/api/admin/bank-review/:id/candidates',owner,safe((req,res)=>{
  const row=db.prepare("SELECT * FROM bank_review WHERE id=? AND environment='test'").get(z.coerce.number().int().positive().parse(req.params.id)) as any;
 ...
 app.patch('/api/admin/bank-review/:id',owner,safe((req,res)=>{
  const id=z.coerce.number().int().positive().parse(req.params.id);
  const p=z.object({state:z.enum(['pending','expense','payment','customer','transfer','card','loan','other']),targetId:z.number().int().positive().nullable(),memo:z.string().max(500)}).parse(req.body);
  db.transaction(()=>{
   const row=db.prepare("SELECT * FROM bank_review WHERE id=? AND environment='test'").get(id) as any;if(!row)throw new Error('내역이 없습니다.');
   if(db.prepare('SELECT id FROM bank_test_postings WHERE bank_id=? AND cancelled_at IS NULL').get(id))throw new Error('테스트 반영을 먼저 취소해 주세요.');
   if(['expense','payment'].includes(p.state)){
    ...
    if(db.prepare("SELECT id FROM bank_review WHERE environment='test' AND state=? AND target_id=? AND id<>?").get(p.state,p.targetId,id))throw new Error('이미 다른 통장 내역과 연결한 기록입니다.');
